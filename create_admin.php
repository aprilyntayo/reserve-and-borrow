...
// Allow super_admin and admin to create accounts
if (
    isset($_SESSION['role']) &&
    $_SESSION['role'] !== 'admin' &&
    $_SESSION['role'] !== 'super_admin'
) {
    die("Access denied! Only admins can create accounts.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$uname || !$fullname || !$email || !$password) {
        ...
        if ($result->num_rows > 0) {
            ...
            $insert = $conn->prepare("
                INSERT INTO users
                (uname, fullname, email, password, role, profile_picture, created_at)
                VALUES (?, ?, ?, ?, ?, ?, NOW())
            ");

            $insert->bind_param("ssssss", $uname, $fullname, $email, $hashed_password, $role, $profile_picture);
...
<head>
    ...
    <title>User Management — AssetEase</title>
...
/* Role cards */
.role-cards { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; }
.role-card {
    ...
    <div class="content">
        ...
        <div class="page-heading">
            <div class="page-heading-left">
                ...
                <p>Register a new user, admin, or super admin for the system.</p>
            </div>
        </div>
                        ...
                        <div class="role-cards">
                            <label class="role-card <?= (($_POST['role'] ?? 'user') === 'user') ? 'selected' : '' ?>" id="rcUser">
                                ...
                                <div class="role-card-body">
                                    ...
                                    <div class="rc-desc">Can browse and submit room reservation requests.</div>
                                </div>
                            </label>
                            ...
                            <label class="role-card <?= (($_POST['role'] ?? '') === 'super_admin') ? 'selected' : '' ?>" id="rcSuper">
                                <input type="radio" name="role" value="super_admin" <?= (($_POST['role'] ?? '') === 'super_admin') ? 'checked' : '' ?> onchange="selectRole(this)">
                                <div class="role-card-icon rc-super-icon"><i class="fas fa-crown"></i></div>
                                <div class="role-card-body">
                                    <div class="rc-name">Super Admin</div>
                                    <div class="rc-desc">Full system control — users, rooms, settings.</div>
                                </div>
                            </label>
                        </div>
                    </div>
